...
void pmm_init(void)
{
	...
	for (uint8_t o = 0; o < MAX_ORDER; o++) {
		if (free_lists[o].count)
			klogv("order[%u]=%llu", o, free_lists[o].count);
	}
}
...
void pmm_free(page_t *page)
{
	...
	if (page_ref_dec_and_test(page)) {
		...
		klogvv("free pfn=0x%llx phys=0x%llx order=%u",
			   (unsigned long long)pfn, (unsigned long long)page_to_phys(page),
			   order);
		...
		free_page_count += count;
		free_block(page);
		klogvv("freed pfn=0x%llx order=%u free=%llu",
			   (unsigned long long)pfn, order, free_page_count);
	}
}
